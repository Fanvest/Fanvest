...
async function main() {
    ...
    // Get default values with error handling
    let defaultPrice, factoryFee;
    try {
        defaultPrice = await factory.defaultTokenPrice();
        factoryFee = await factory.factoryFee();
    } catch (error) {
        console.log("⚠️  Erreur lors de la lecture des valeurs par défaut, utilisation de valeurs fixes");
        defaultPrice = ethers.parseEther("0.001");
        factoryFee = ethers.parseEther("0.01");
    }
